...
namespace greaper::StringUtils
{
		...
		const BasicString<T, _SAlloc_>& separator) noexcept
	{
		...
		static_assert(std::is_same_v<T, typename _SAlloc_::value_type>, "Trying to ComposeString and convert a string at the same time");
		
		BasicString<T, _Alloc_> rtn;
		for (sizet i = 0; i < vec.size(); ++i)
		{
			rtn += vec[i];
			if (i != (vec.size() - 1))
	...
	std::vector<BasicString<T, _Alloc_>, _VAlloc_> Tokenize(const BasicString<T, _Alloc_>& str, T token) noexcept
	{
		...
		std::vector<BasicString<T, _Alloc_>, _VAlloc_> rtn;
		if (str.empty())
			return rtn;
		...
		rtn.push_back(BasicString<T, _Alloc_>(lastToken, str.end()));
		return rtn;
	}
		...
		const BasicString<T, _TAlloc_>& token) noexcept
	{
		...
		static_assert(std::is_same_v<T, typename _VAlloc_::value_type::allocator_type::value_type>, "Bad StringVec allocator type");
	...
	std::vector<BasicString<T, _Alloc_>, _VAlloc_> SeparateBySpace(const BasicString<T, _Alloc_>& str) noexcept
	{
		...
		static_assert(std::is_same_v<T, typename _VAlloc_::value_type::allocator_type::value_type>, "Bad StringVec allocator");
	...
	BasicString<T, _Alloc_>& TrimSelf(BasicString<T, _Alloc_>& str, bool left = true, bool right = true) noexcept
	{
		...
		static Vector<T, _Alloc_> delims { T(' '), T('\t'), T('\r'), T('\n') };
	...
	BasicString<T, _Alloc_> Replace(BasicStringView<T> str, T toReplace, T replacement) noexcept
	{
		...
		return ComposeString<T, _Alloc_>(Tokenize<T, _Alloc_>(str, toReplace), replacement);
	}
		...
		const BasicString<T, _TAlloc_>& toReplace, const BasicString<T, _RAlloc_>& replacement) noexcept
	{
		...
		static_assert(std::is_same_v<T, typename _RAlloc_::value_type>, "Bad string allocator type");
	...
	BasicString<T, _Alloc_> ToLower(const BasicString<T, _Alloc_>& str) noexcept
	{
		...
		BasicString<T, _Alloc_> temp{ str.c_str(), str.size() };
